...
public abstract class Command implements CommandExecutor
{
	...
	protected final int argAsInt(int index, boolean msg)
	{
		if (args.length <= index)
			throw new CommandException(CommandException.Reason.SYNTAX);
		...
		{
			if (msg) throw new CommandException(CommandException.Reason.INPUT, "&c{0} &4is not a number.", arg);
			else return -1;
		}
	}
	...
	{
		if (value == null)
			throw new CommandException(CommandException.Reason.VALIDATE, message, args);
		return value;
	}
	...
	protected void checkPermission(CommandSender sender, IPermission permission)
	{
		if (! hasPermission(sender, permission, true))
			throw new CommandException(CommandException.Reason.BREAK);
	}
